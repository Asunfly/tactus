...
- Tables when presenting structured data`;
  ...
  while (iteration < maxIterations) {
    ...
    const effectiveAnthropicTools = toolLoopState.toolUseDisabled ? undefined : anthropicTools;
    if (effectiveAnthropicTools && effectiveAnthropicTools.length > 0) {
      requestBody.tools = effectiveAnthropicTools;
